...
class ReadingTest extends TestCase
{
    ...
    public function test_un_chapitre_inexistant_est_refuse(): void
    {
        ...
        $this->actingAs($user)
            ->postJson('/api/readings', ['book_id' => 1, 'chapter' => 51])
            ->assertUnprocessable()
    ...
    public function test_un_livre_inexistant_est_refuse(): void
    {
        $this->actingAs($this->member())
            ->postJson('/api/readings', ['book_id' => 67, 'chapter' => 1])
            ->assertUnprocessable()
    ...
    public function test_la_liste_ne_renvoie_que_les_chapitres_du_cycle_en_cours(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1]);
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 1, 'chapter' => 2]);
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 19, 'chapter' => 23]);
    ...
    public function test_mediter_rapporte_plus_que_lire(): void
    {
        ...
        $this->actingAs($user)
            ->postJson('/api/meditations', ['book_id' => 43, 'chapter' => 3])
            ->assertCreated()
    ...
    public function test_un_quiz_peut_etre_rejoue_et_rapporte_a_chaque_fois(): void
    {
        ...
        foreach (range(1, 3) as $ignored) {
            $this->actingAs($user)->postJson('/api/quiz-attempts', [
                'scope' => 'general',
                'score' => 7,
                'total' => 10,
            ])->assertCreated();
    ...
    {
        $this->actingAs($this->member())->postJson('/api/quiz-attempts', [
            'scope' => 'general',
            'score' => 12,
            'total' => 10,
    ...
    public function test_le_niveau_progresse_avec_les_points(): void
    {
        ...
        foreach (range(1, 30) as $chapter) {
            $this->actingAs($user)->postJson('/api/meditations', [
