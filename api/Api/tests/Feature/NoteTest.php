...
class NoteTest extends TestCase
{
    ...
    {
        $this->actingAs($this->member())
            ->postJson('/api/notes', ['content' => 'Penser a prier pour Aminata.'])
            ->assertCreated()
    ...
    {
        $this->actingAs($this->member())
            ->postJson('/api/notes', ['content' => 'Une note.', 'chapter' => 3])
            ->assertUnprocessable()
    ...
    public function test_personne_ne_peut_modifier_la_note_d_un_autre(): void
    {
        ...
        $this->actingAs($this->member())
            ->putJson("/api/notes/{$note->id}", ['content' => 'Texte modifie.'])
            ->assertForbidden();
    ...
    public function test_la_liste_ne_contient_que_ses_propres_notes(): void
    {
        ...
        $this->actingAs($moi)->getJson('/api/notes')
            ->assertOk()
    ...
    public function test_les_notes_se_filtrent_par_passage(): void
    {
        ...
        $this->actingAs($moi)->getJson('/api/notes?book_id=43')
            ->assertOk()
            ->assertJsonCount(1, 'data')
