...
class ReadingCycleTest extends TestCase
{
    ...
    public function test_relire_un_chapitre_est_toujours_possible_et_compte(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1])
            ->assertCreated()
    ...
    public function test_le_nombre_de_relectures_est_expose(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 43, 'chapter' => 3]);
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 43, 'chapter' => 3]);

        $this->actingAs($user)->getJson('/api/readings')
            ->assertOk()
    ...
    public function test_decocher_un_chapitre_le_retire_de_la_liste(): void
    {
        ...
        $this->actingAs($user)
            ->deleteJson('/api/readings', ['book_id' => 1, 'chapter' => 1])
            ->assertOk()
            ->assertJsonPath('progress.chapters_read', 0);

        $this->actingAs($user)->getJson('/api/readings')->assertJsonPath('count', 0);
    }
    ...
    public function test_decocher_ne_reprend_pas_les_points_deja_gagnes(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1]);
        $this->actingAs($user)->deleteJson('/api/readings', ['book_id' => 1, 'chapter' => 1]);
    ...
    public function test_relire_recoche_automatiquement(): void
    {
        ...
        $this->actingAs($user)->getJson('/api/readings')->assertJsonPath('count', 1);
    }
    ...
    public function test_on_peut_recommencer_sans_avoir_tout_lu(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/progress/reset-reading')
            ->assertOk()
    ...
    public function test_un_cycle_vide_ne_peut_pas_etre_reinitialise(): void
    {
        $this->actingAs($this->member())
            ->postJson('/api/progress/reset-reading')
            ->assertStatus(422)
    ...
    public function test_un_delai_separe_deux_reinitialisations(): void
    {
        ...
        $this->actingAs($user)->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1]);
        $this->actingAs($user)->postJson('/api/progress/reset-reading')->assertOk();

        $this->actingAs($user->refresh())->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1]);
    ...
    public function test_relire_dans_le_nouveau_cycle_rapporte_a_nouveau(): void
    {
        ...
        $this->actingAs($user->refresh())
            ->postJson('/api/readings', ['book_id' => 1, 'chapter' => 1])
            ->assertCreated()
    ...
    public function test_le_nombre_de_bibles_lues_est_deduit_du_cycle(): void
    {
        ...
        $this->actingAs($user->refresh())->getJson('/api/progress')
