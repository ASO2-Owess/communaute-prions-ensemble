...
class PastoralTest extends TestCase
{
    ...
    public function test_un_membre_peut_poser_une_question(): void
    {
        $this->actingAs($this->member())
            ->postJson('/api/v1/questions', ['body' => self::TEXTE, 'topic' => 'vocation'])
            ->assertCreated()
    ...
    {
        $this->actingAs($this->member())
            ->postJson('/api/v1/questions', ['body' => 'aide moi'])
            ->assertUnprocessable()
    ...
    public function test_la_liste_ne_contient_que_ses_propres_questions(): void
    {
        ...
        $this->actingAs($auteur)->getJson('/api/v1/questions')
            ->assertOk()
    ...
    public function test_un_membre_ne_peut_pas_saturer_la_file(): void
    {
        ...
        foreach (range(1, $max) as $ignored) {
            $this->actingAs($user)
                ->postJson('/api/v1/questions', ['body' => self::TEXTE])
                ->assertCreated();
        }
    ...
    {
        $this->actingAs($this->member())
            ->getJson('/api/v1/pastor/questions')
            ->assertForbidden();

        $this->actingAs($this->member())
            ->getJson('/api/v1/pastor/stats')
            ->assertForbidden();
    }
    ...
    public function test_le_pasteur_voit_la_file_du_plus_ancien_au_plus_recent(): void
    {
        ...
        $this->actingAs($this->pastor())->getJson('/api/v1/pastor/questions')
            ->assertOk()
    ...
    public function test_un_brouillon_reste_invisible_pour_le_membre(): void
    {
        ...
        $this->actingAs($membre)->getJson("/api/v1/questions/{$question->id}")
            ->assertOk()
    ...
    public function test_prendre_en_charge_change_le_statut(): void
    {
        ...
        $this->actingAs($pasteur)
            ->postJson("/api/v1/pastor/questions/{$question->id}/claim")
            ->assertOk()
